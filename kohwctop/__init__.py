...
@dataclass
class KoCtoPTrainConfig(TrainConfig):
    pass

@dataclass
class ConvNetTrainConfig(TrainConfig):
    epoch: int = 3

@dataclass
class TestConfig:
    batch_size: int = 50   # 묶어서 테스트할 숫자 (batch size)
...
class ModelConfig:
    ...
    dropout: float = 0.5                                # 드롭아웃 비율 (매 학습 때마다 쉬는 노드 비율)

@dataclass
class KoCtoPConfig(ModelConfig):
    pass
